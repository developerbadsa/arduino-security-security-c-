...
bool wifiHttpPost(const char* url, const String& json, String* outBody) {
  ...
  if (!http.begin(wifiClient, url)) {
    LOG_PRINTLN_IF(DBG_NET, "WIFI", F("HTTPClient begin failed"));
    return false;
  }

  http.addHeader("Content-Type", "application/json");
  http.setUserAgent("SmartBike-WiFi/9.1");
  ...
  if (httpCode < 0) {
    LOG_PRINTF_IF(DBG_NET, "WIFI", "POST failed err=%s", http.errorToString(httpCode).c_str());
    http.end();
    return false;
  }
...
void wifiFallbackMaintenance() {
  const bool gsmUp = modemReady && bearerReady;

  // If GSM is up, turn off WiFi fallback
  if (gsmUp) {
    if (wifiFallbackActive) {
      LOG_PRINTLN_IF(DBG_NET, "WIFI", F("GSM restored, disabling WiFi fallback"));
  ...
  // GSM is down — check if WiFi is already connected
  if (WiFi.status() == WL_CONNECTED) {
    if (!wifiFallbackActive) {
      wifiFallbackActive = true;
      LOG_PRINTF_IF(DBG_NET, "WIFI", "Fallback connected ip=%s", WiFi.localIP().toString().c_str());
    }
    return;
  }

  // WiFi not connected — respect retry interval
  if (millis() - lastWifiFallbackTry < WIFI_RETRY_INTERVAL_MS) return;
  lastWifiFallbackTry = millis();

  wifiFallbackActive = false;
  LOG_PRINTLN_IF(DBG_NET, "WIFI", F("GSM down, attempting WiFi fallback..."));
  ...
  LOG_PRINTLN_IF(DBG_NET, "WIFI", F("Fallback connect timeout"));
  WiFi.disconnect(true);
  WiFi.mode(WIFI_OFF);
}

bool httpPostJSON(const char* url, const String& json, String* outBody) {
  // Route through WiFi fallback if active
  if (wifiFallbackActive && WiFi.status() == WL_CONNECTED) {
    return wifiHttpPost(url, json, outBody);
  }
...
bool netIsConnected() {
  return (modemReady && bearerReady) || wifiFallbackActive;
}
