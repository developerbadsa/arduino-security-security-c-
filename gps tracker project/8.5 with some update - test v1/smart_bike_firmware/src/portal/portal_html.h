...
static const char HTML_PAGE[] PROGMEM =
  ...
  ".sectionTitle{font-size:13px;font-weight:900;margin-bottom:10px}"
  ".logsHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px}"
  ".logsMeta{font-size:12px;color:var(--muted)}"
  ".logsList{display:flex;flex-direction:column;gap:6px;max-height:430px;overflow:auto;padding-right:4px}"
  ".logItem{display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:12px;"
  "border:1px solid var(--line);background:rgba(255,255,255,.03);font-size:11px;line-height:1.2}"
  ".logAge{flex:0 0 34px;color:var(--warn);font-weight:900}"
  ".logText{flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--txt)}"
  ".logEmpty{padding:12px;border:1px dashed var(--line);border-radius:12px;color:var(--muted);font-size:12px;"
  "text-align:center}"
  "</style></head><body><div class='wrap'>"
  "<div class='top'>"
  "<div class='title'>&#x1F6B2; Bike Local Control</div>"
  "<div class='chips'>"
  "<span class='chip' id='chipAp'><span class='dot bad'></span><span>Local AP</span><b id='apTxt'>OFF</b></span>"
  "<span class='chip' id='chipNet'><span class='dot bad'></span><span>GSM Net</span><b id='netTxt'>BAD</b></span>"
  ...
  "<div class='logsMeta' id='logsMeta'>0/16 recent</div>"
  "</div>"
  "<div class='logsList' id='logsList'><div class='logEmpty'>No logs yet.</div></div>"
  "</div>"
  "<script>"
  ...
  "function renderLogs(list,now){const box=document.getElementById('logsList');"
  "const items=Array.isArray(list)?list:[];"
  "document.getElementById('logsMeta').textContent=items.length+'/16 recent';"
  "if(!items.length){box.innerHTML=\"<div class='logEmpty'>No logs yet.</div>\";return;}"
  "box.innerHTML=items.slice().reverse().map(x=>{"
  "const age=Math.max(0,Math.floor(((now||0)-(x.ms||0))/1000));"
  "return \"<div class='logItem'><span class='logAge'>\"+age+\"s</span><span class='logText'>\"+esc(x.text||'-')+\"</span></div>\";"
  "}).join('');}"
  ...
  "document.getElementById('apTxt').textContent=j.ap?'ON':'OFF';"
  "document.getElementById('netTxt').textContent=j.net?'OK':'BAD';"
  "setChip('chipAp',j.ap?'ok':'bad');setChip('chipNet',j.net?'ok':'bad');"
